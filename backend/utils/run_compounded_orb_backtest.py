...
async def run_simulation():
    ...
    broker = PaperBroker(initial_cash_inr=initial_capital, product_type="INTRADAY", latency_ms=0.0, sim_cfg=sim_cfg)
    risk = RiskController(
        max_capital_per_trade_inr=initial_capital * leverage * 10000.0,
        max_daily_loss_inr=float("inf"),
    ...
    for symbol in symbols_with_data:
        ...
        strat.capital_limit = initial_capital
        strat.leverage = leverage
        
        manager.register_strategy(strat)

    print(f"Running COMPOUNDED simulation timeline (Dynamic Sizing based on running P/L)...")
    ...
    for idx, r in enumerate(all_rows):
        # Dynamically update available capital to match running equity/cash for compounding
        current_cash = broker.get_portfolio()["cash_inr"]
        manager.total_capital = current_cash
        risk.max_capital_per_trade_inr = current_cash * leverage * 2.0
    ...
    # Master output directory for compounded backtest
    output_dir = backend_dir.parent / "market_data" / "compounded_orb"
    ...
    perf_summary = {
        "Total Portfolio Capital (INR)": initial_capital,
    ...
    print("==================================================")
    print("   MASTER CONSOLIDATED PERFORMANCE REPORT (COMPOUNDED ORB)")
    print("==================================================")
