...
async def main():
    nifty201_250_csv = backend_dir.parent / "market_data" / "history" / "nifty_201-250" / "nifty_201-250.csv"
    if not nifty201_250_csv.exists():
        print(f"ERROR: Nifty 201-250 symbols list not found at: {nifty201_250_csv}")
        sys.exit(1)
        
    market_dir = backend_dir.parent / "market_data" / "history" / "nifty_201-250"
    market_dir.mkdir(parents=True, exist_ok=True)
    
    # Read symbols
    symbols = []
    with open(nifty201_250_csv, mode="r", encoding="utf-8") as f:
        reader = csv.reader(f)
    ...
    # Save mapping
    mapping_csv = backend_dir.parent / "market_data" / "nifty201_250_security_ids.csv"
