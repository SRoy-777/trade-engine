...
async def download_stock_data(symbol: str, security_id: str, start_date: date, end_date: date, market_dir: Path, sem: asyncio.Semaphore) -> bool:
    ...
    # Clean symbol for filename to avoid issue with ampersand or special chars
    # However, keeping symbol exactly as trading symbol is best.
    # The file system handles ampersand fine on Windows.
    # In case the file name contains & (e.g. M&M), Windows handles it without any issues.
...
async def main():
    nifty301_350_csv = backend_dir.parent / "market_data" / "history" / "nifty_301-350" / "nifty_301-350.csv"
    if not nifty301_350_csv.exists():
        print(f"ERROR: Nifty 301-350 symbols list not found at: {nifty301_350_csv}")
        sys.exit(1)
        
    market_dir = backend_dir.parent / "market_data" / "history" / "nifty_301-350"
    market_dir.mkdir(parents=True, exist_ok=True)
    
    # Read symbols
    symbols = []
    with open(nifty301_350_csv, mode="r", encoding="utf-8") as f:
        reader = csv.reader(f)
    ...
    # Save mapping
    mapping_csv = backend_dir.parent / "market_data" / "nifty301_350_security_ids.csv"
