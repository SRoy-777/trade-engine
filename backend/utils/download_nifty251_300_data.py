...
async def main():
    nifty251_300_csv = backend_dir.parent / "market_data" / "history" / "nifty_251-300" / "nifty_251-300.csv"
    if not nifty251_300_csv.exists():
        print(f"ERROR: Nifty 251-300 symbols list not found at: {nifty251_300_csv}")
        sys.exit(1)
        
    market_dir = backend_dir.parent / "market_data" / "history" / "nifty_251-300"
    market_dir.mkdir(parents=True, exist_ok=True)
    
    # Read symbols
    symbols = []
    with open(nifty251_300_csv, mode="r", encoding="utf-8") as f:
        reader = csv.reader(f)
    ...
    # Save mapping
    mapping_csv = backend_dir.parent / "market_data" / "nifty251_300_security_ids.csv"
