...
def clean_timestamp(ts_val: float) -> datetime:
    # Convert epoch to local datetime naive
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=15) as response:
                ...
                if "timestamp" in res_data:
                    ...
                    # In case of weekend or holidays, no candle data might be returned, which is fine
                    pass
...
async def main():
    ...
    if not nifty_csv_path.exists():
        print(f"ERROR: Nifty 50 ID mapping not found at: {nifty_csv_path}")
        sys.exit(1)
    ...
    print(f"Found {len(stocks)} stocks to download from {start_date} to {end_date}...\n")
