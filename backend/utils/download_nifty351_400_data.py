...
async def main():
    nifty351_400_csv = backend_dir.parent / "market_data" / "history" / "nifty_351-400" / "nifty_351-400.csv"
    if not nifty351_400_csv.exists():
        print(f"ERROR: Nifty 351-400 symbols list not found at: {nifty351_400_csv}")
        sys.exit(1)
        
    market_dir = backend_dir.parent / "market_data" / "history" / "nifty_351-400"
    market_dir.mkdir(parents=True, exist_ok=True)
    
    # Read symbols
    symbols = []
    with open(nifty351_400_csv, mode="r", encoding="utf-8") as f:
        reader = csv.reader(f)
    ...
    # Save mapping
    mapping_csv = backend_dir.parent / "market_data" / "nifty351_400_security_ids.csv"
