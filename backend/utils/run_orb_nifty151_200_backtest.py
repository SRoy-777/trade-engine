...
async def run_single_stock_backtest(symbol: str, config_path: Path, config: dict, start_dt: date, end_dt: date) -> tuple:
    ...
    # Locate data file in nifty_151-200 historical directory
    history_dir = backend_dir.parent / "market_data" / "history" / "nifty_151-200"
    csv_paths = [
    ...
    if not csv_path:
        print(f"  [SKIPPED] Historical data not found for {symbol} in nifty_151-200 folder")
        return [], []
    ...
    # Individual stock output goes to nifty_151-200 subfolder
    individual_output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_151-200" / symbol
...
async def run_simulation():
    ...
    config_path = backend_dir.parent / "configs" / "orb_nifty_151_200.yaml"

    print(f"Loading ORB config from: {config_path}")
    if not config_path.exists():
        print(f"Error: ORB Nifty 151-200 config not found at {config_path}")
        return
    ...
    if isinstance(config_symbols, list):
        ...
        if config_symbols.upper() == "ALL":
            nifty_csv_path = backend_dir.parent / "market_data" / "nifty151_200_security_ids.csv"
    ...
    print(f"Running simulation for {len(symbols)} Nifty 151-200 symbols from {start_dt} to {end_dt}...")
    ...
    print("\nConsolidating daily equity across all Nifty 151-200 symbols...")
    unique_dates = set()
    ...
    # Output dir for nifty_151-200 consolidated reports
    output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_151-200"
    output_dir.mkdir(parents=True, exist_ok=True)
