...
async def run_single_stock_backtest(symbol: str, config_path: Path, config: dict, start_dt: date, end_dt: date) -> tuple:
    ...
    # Locate data file in nifty_301-350 historical directory
    history_dir = backend_dir.parent / "market_data" / "history" / "nifty_301-350"
    csv_paths = [
    ...
    if not csv_path:
        print(f"  [SKIPPED] Historical data not found for {symbol} in nifty_301-350 folder")
        return [], []
    ...
    # Individual stock output goes to nifty_301-350 subfolder
    individual_output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_301-350" / symbol
...
async def run_simulation():
    ...
    config_path = backend_dir.parent / "configs" / "orb_nifty_301_350.yaml"

    print(f"Loading ORB config from: {config_path}")
    if not config_path.exists():
        print(f"Error: ORB Nifty 301-350 config not found at {config_path}")
        return
    ...
    if isinstance(config_symbols, list):
        ...
        if config_symbols.upper() == "ALL":
            nifty_csv_path = backend_dir.parent / "market_data" / "nifty301_350_security_ids.csv"
    ...
    print(f"Running simulation for {len(symbols)} Nifty 301-350 symbols from {start_dt} to {end_dt}...")
    ...
    print("\nConsolidating daily equity across all Nifty 301-350 symbols...")
    unique_dates = set()
    ...
    # Output dir for nifty_301-350 consolidated reports
    output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_301-350"
    output_dir.mkdir(parents=True, exist_ok=True)
