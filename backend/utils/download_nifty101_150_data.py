...
async def main():
    nifty101_150_csv = backend_dir.parent / "market_data" / "history" / "nifty_101-150" / "nifty_101-150.csv"
    if not nifty101_150_csv.exists():
        print(f"ERROR: Nifty 101-150 symbols list not found at: {nifty101_150_csv}")
        sys.exit(1)
        
    market_dir = backend_dir.parent / "market_data" / "history" / "nifty_101-150"
    market_dir.mkdir(parents=True, exist_ok=True)
    
    # Read symbols
    symbols = []
    with open(nifty101_150_csv, mode="r", encoding="utf-8") as f:
        reader = csv.reader(f)
    ...
    # Save mapping
    mapping_csv = backend_dir.parent / "market_data" / "nifty101_150_security_ids.csv"
