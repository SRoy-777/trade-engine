...
async def run_single_stock_backtest(symbol: str, config_path: Path, config: dict, start_dt: date, end_dt: date) -> tuple:
    ...
    # Locate data file in nifty_101-150 historical directory
    history_dir = backend_dir.parent / "market_data" / "history" / "nifty_101-150"
    csv_paths = [
    ...
    if not csv_path:
        print(f"  [SKIPPED] Historical data not found for {symbol} in nifty_101-150 folder")
        return [], []
    ...
    # Individual stock output goes to nifty_101-150 subfolder
    individual_output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_101-150" / symbol
...
async def run_simulation():
    ...
    config_path = backend_dir.parent / "configs" / "orb_nifty_101_150.yaml"

    print(f"Loading ORB config from: {config_path}")
    if not config_path.exists():
        print(f"Error: ORB Nifty 101-150 config not found at {config_path}")
        return
    ...
    if isinstance(config_symbols, list):
        ...
        if config_symbols.upper() == "ALL":
            nifty_csv_path = backend_dir.parent / "market_data" / "nifty101_150_security_ids.csv"
    ...
    capital = float(config.get("capital", 60000.0))
    start_date_str = config.get("start_date", "2026-05-01")
    end_date_str = config.get("end_date", "2026-07-06")
    start_dt = datetime.strptime(start_date_str, "%Y-%m-%d").date()
    end_dt = datetime.strptime(end_date_str, "%Y-%m-%d").date()

    print(f"Running simulation for {len(symbols)} Nifty 101-150 symbols from {start_dt} to {end_dt}...")
    ...
    print("\nConsolidating daily equity across all Nifty 101-150 symbols...")
    unique_dates = set()
    ...
    # Output dir for nifty_101-150 consolidated reports
    output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_101-150"
    output_dir.mkdir(parents=True, exist_ok=True)
