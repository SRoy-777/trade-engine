...
async def run_single_stock_backtest(symbol: str, config_path: Path, config: dict, start_dt: date, end_dt: date) -> tuple:
    ...
    # Locate data file in nifty_451-500 historical directory
    history_dir = backend_dir.parent / "market_data" / "history" / "nifty_451-500"
    csv_paths = [
    ...
    if not csv_path:
        print(f"  [SKIPPED] Historical data not found for {symbol} in nifty_451-500 folder")
        return [], []
    ...
    # Individual stock output goes to nifty_451-500 subfolder
    individual_output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_451-500" / symbol
...
async def run_simulation():
    ...
    config_path = backend_dir.parent / "configs" / "orb_nifty_451_500.yaml"

    print(f"Loading ORB config from: {config_path}")
    if not config_path.exists():
        print(f"Error: ORB Nifty 451-500 config not found at {config_path}")
        return
    ...
    if isinstance(config_symbols, list):
        ...
        if config_symbols.upper() == "ALL":
            nifty_csv_path = backend_dir.parent / "market_data" / "nifty451_500_security_ids.csv"
    ...
    print(f"Running simulation for {len(symbols)} Nifty 451-500 symbols from {start_dt} to {end_dt}...")
    ...
    print("\nConsolidating daily equity across all Nifty 451-500 symbols...")
    unique_dates = set()
    ...
    # Output dir for nifty_451-500 consolidated reports
    output_dir = backend_dir.parent / "market_data" / "orb" / "nifty_451-500"
    output_dir.mkdir(parents=True, exist_ok=True)
